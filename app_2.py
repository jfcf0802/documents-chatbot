...
# Generate a response based on the document text and user query using question-answering model
def generate_response(query, document_text):
    # Use the QA pipeline to extract answer from the document
    if not query.strip():
        return "Please ask a question about the document."

    try:
        result = qa_pipeline(question=query, context=document_text)
        answer = result.get('answer', 'Sorry, I could not find an answer in the document.')
        return answer
    except Exception as e:
